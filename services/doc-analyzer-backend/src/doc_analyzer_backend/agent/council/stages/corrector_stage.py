...
async def run_corrector_stage(
    ...
    logger.info(
        f"Council of {len(correctors)} CORRECTOR agents: correction is starting..."
    )
    ...
    logger.info(
        f"Council of {len(correctors)} CORRECTOR agents: correction is completed"
    )
